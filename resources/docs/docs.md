...
The `home-routes` handler in the `gd-backend.routes.home` namespace
...
The `home-page` function will in turn call the `gd-backend.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `gd-backend.handler` namespace:
...
Request middleware functions are located under the `gd-backend.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `gd-backend.dev-middleware` namespace found in
