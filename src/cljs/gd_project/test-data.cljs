...
(def columns [{:title "区" :dataIndex "unit" :sorter #(comparison %1 %2 :unit)}
              {:title "地址" :dataIndex "main_category" :sorter #(comparison %1 %2 :main_category)}
              {:title "状态" :dataIndex "status" :sorter #(comparison %1 %2 :room )}])
